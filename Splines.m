...
%read data
x = [1.25 1.60 2.21 2.78 3.60 4.26 4.82...
    5.50 6.21 6.88 7.58 8.31 8.93];
y = [1.15 1.48 1.71 1.83 2.17 2.47 2.64...
...
b=zeros(12,1);
for l=1:12
b(l)=((y(l+1)-y(l))/(x(l+1)-x(l)))-((2/3)*c_ans(l)*(x(l+1)-x(l)))-...
...
ytotal=0;
for t = 1:12
   f =  @(q) y(t)+b(t).*(q-x(t))+c_ans(t).*(q-x(t)).^2+d(t)*(q-x(t)).^3;
